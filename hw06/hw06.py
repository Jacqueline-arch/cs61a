...
class VendingMachine:
    ...
    def __init__(self, product, unit_price):
        ...
        self.balance = 0
        self.stock = 0
        self.uni_price = unit_price
    
    def vend(self):
        if self.stock == 0 and self.balance == 0:
            return f'Nothing left to vend. Please restock.'
        if self.stock > 0 and self.balance < self.uni_price:
            return f'Please add ${self.uni_price - self.balance} more funds.'
        if self.stock > 0 and self.balance == self.uni_price:
            self.balance = 0
            self.stock -= 1
            return f'Here is your {self.name}.'
        if self.stock > 0 and self.balance > self.uni_price:
            self.change = self.balance - self.uni_price
            self.balance = 0
            self.stock -= 1
    ...
    def add_funds(self, funds):
        ...
        if self.stock == 0:
            ...
            self.balance = 0
            return f'Nothing left to vend. Please restock. Here is your ${self.change}.'
        else:
...
class Player:
    ...
    def debate(self, other):
        self.probability = max(0.1, self.popularity/(self.popularity + other.popularity))
...
class Game:
    ...
    def play(self):
        while not self.game_over():
            if self.turn % 2 == 0:
                other = self.p2
                self.p1.choose(other)(other)
            else:
                other = self.p1
                self.p2.choose(other)(other)
            self.turn += 1
...
class VirFib():
    ...
    def __init__(self, value=0, next1 = 1):
        self.value = value
        self.next1 = next1

    def next(self):
        return VirFib(self.next1, self.value + self.next1 )
