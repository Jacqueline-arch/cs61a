...
class AICard(Card):
    ...
    def effect(self, opponent_card: Card, player: Player, opponent: Player):
        ...
        True
        """
        first = player.deck.cards.pop(0)
        second = player.deck.cards.pop(1)
...
class TutorCard(Card):
    ...
    def power(self, oppponent_card: Card):
        return oppponent_card.power(self) - 1
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(player.hand):
            ...
            best_power = player.hand[0].power(opponent_card)
            for i in player.hand:
                if i.power(opponent_card) > best_power:
                    best_card = i
