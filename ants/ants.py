...
class Place:
    ...
    def __init__(self, name, exit: 'Place' = None):
...
class HarvesterAnt(Ant):
    ...
    def action(self, gamestate:'GameState'):
        ...
        """
        # BEGIN Problem 1
        gamestate.food += self.food_provide
        # END Problem 1
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if len(place.bees) > 0 and self.lower_bound <= i <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        place_bee = self.place
        super().reduce_health(amount)
        bee_list = place_bee.bees[:]
        for i in bee_list:
            if not self.place:
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while place != None:
            if place.ant != None:
                if not place.ant.damage_double:
                    place.ant.double()
                if place.ant.is_container and place.ant.ant_contained and not place.ant.ant_contained.damage_double:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # print('DEBUG1:', self.place)
        # print('DEBUG:', self.place.exit)
        if self.blocked():
            # print('DEBUG:', self.blocked())
            self.sting(self.place.ant)
        elif self.health > 0 and destination is not None:
            self.move_to(destination)
        # print('DEBUG2:', self.place)
    def add_to(self, place):
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not place.entrance.is_hive:
            place_list.append(place.entrance)  
            distance += 1
            place = place.entrance
        # print('DEBUG:', place_list)
        for i in place_list:
            if i.ant:
                dic[i.ant] = place_list.index(i) + 1
                if i.ant.is_container and i.ant.ant_contained:
                    dic[i.ant.ant_contained] = place_list.index(i) + 1
            if i.bees:
                # print('DEBUG:', i.bees)
                for h in i.bees:
                    dic[h] = place_list.index(i) + 1
    ...
    def action(self, gamestate):
        ...
        for insect, distance in insects_and_distances.items():
            ...
            if damage:
                self.insects_shot += 1
            print('DEBUG dead:', self.insects_shot)
