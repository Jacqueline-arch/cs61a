...
def do_lambda_form(expressions: Pair, env: Frame) -> LambdaProcedure: 
...
def make_let_frame(bindings: Pair, env: Frame)-> Frame:
    ...
    while bindings is not nil:
        ...
        exp = bindings.first.rest.first
        print("DEBUG:", isinstance(exp, Pair))
...
def do_mu_form(expressions: Pair, env: Frame) -> MuProcedure:
    """Evaluate a mu form."""
    validate_form(expressions, 2)
    formals = expressions.first
    validate_formals(formals)
